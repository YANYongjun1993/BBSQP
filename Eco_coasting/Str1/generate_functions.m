...
rw       = 0.364;%车轮半径
Ig       = 2.757;%4挡位传动比
w        = 30*Ig/(3.14*rw*1000); 
% p1       =    0.2159; % 怠速油耗m/s
% p2       =    0.005676 ;
% p3       =    0.0004349; 
% p4       =    8.899e-07 ; % The coefficients of the fuel consumption model f(x,y) = p1 + p2*x*y + p3*x^2*y + p4*x*y^2 
p1       =    0.372; % 怠速油耗m/s
p2       =    -0.00307;
p3       =    3.4e-05; % The coefficients of the fuel consumption model f(x,y) = p1*x + p2*x^2 + p3*x*y^2
Td       =    30; %发动机反拖力矩_Nm
% Terminal cost
Phi      =    [];

% Incremental cost
L = beta*(1/x)*(p1*w*x*u3 + p2*w^2*x^2 + p3*w*x*u1^2 ) + (1-beta)*(x - r)^2;

% Equations of system dynamics
f = x + ts*( (itat*Ig*u1 - Td*(1-u3) - u2)/(rw*M*x) - cd*Af*ro*x/2/M - g*(fr*cos(t) + sin(t))/x );
...
if isempty(f)
    f = zeros(0,1);
    matlabFunction(f, 'file', 'valf','vars',[x, u, t]); 
else
    matlabFunction(f, 'file', 'valf','vars',{x, u, t}); 
end
...
if isempty(f)
    ...
    matlabFunction(fx, 'file', 'DfDx','vars',{x, u, t});   
    ...
    matlabFunction(fxx, 'file', 'D2fDxx','vars',{x, u, t}); 
