...
for i = 1:nsim-1
    ...
    tic;
    [u_sqp, it] = BandB(xsim(:,i),Num,km,opt,yidx,size_x,size_u, p, z_hist, limit_num);
    time(i) = toc;
    uu = reshape(u_sqp(1:size_u*Num,1),[size_u,Num]);
    z_curr= uu(3,:);
    z_hist(1) = [];
    z_hist(limit_num) = z_curr(1);
%     warmtree = uu(3,:); % Optimal path at current sampling interval
...
load with_horcon_100.mat
% semilogy(t1,res1,'--',t2,res2);
stairs(t2(1:nsim-1),iter(1:nsim-1)-1,'r--');
hold on
load withoutws100.mat
