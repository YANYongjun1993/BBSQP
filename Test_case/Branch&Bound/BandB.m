function x_best = BandB(x0,v0,o,yidx)
% BandB
% Branch and bound algorithm for nonlinear mixed integer programs
...
% 
% Programmed by Yongjun Yan 2021/11/30
%  
...
%         [x,this_f,flag] = lpr(c,A,b,Aeq,beq,e,s,d,yidx,sol,opts);
        [x,this_f,flag] = seq_quad_prog(x0,v0,o,s,d,yidx); 
        x = x'; % Transpose matrix for demension request in "diff = abs(round(y)-y)"
        yidx = logical(yidx);
        %Visit this node
...
%                     s1(branch_idx)=ceil(abs(y(branch_idx)));
...
%                     s2(branch_idx)=floor(abs(y(branch_idx)));
